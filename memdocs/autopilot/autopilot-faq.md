...
|  È necessario diventare un CSP per partecipare a Windows Autopilot? | Questa operazione non è necessaria per i principali OEM del volume perché può usare l'API diretta OEM.  Tutti gli altri utenti che scelgono di usare MPC per registrare i dispositivi devono diventare CSP per poter accedere a MPC.  |
...
| Esiste un unico account CSP globale?   |  No. Le aree di vendita CSP dipendono dalla posizione del tenant (AzureAD).  Un partner CSP può vendere o gestire solo i clienti che dispongono di un tenant situato nella stessa area CSP del partner registrato come CSP (il percorso del tenant usato dal partner CSP per eseguire la transazione). Se il tenant del cliente è stato creato negli Stati Uniti, solo un partner che ha una registrazione CSP negli Stati Uniti può stabilire una relazione di rivenditore con questo cliente. Per quanto riguarda Autopilot & Intune, non è rilevante la posizione in cui si trova l'utente finale o il dispositivo. Un dispositivo usato da un dipendente situato in Germania può registrarsi usando il profilo di Autopilot creato nel tenant degli Stati Uniti e può essere gestito dall'istanza del servizio Intune negli Stati Uniti. L'utente in Germania eseguirà anche l'autenticazione nell'istanza AzureAD degli Stati Uniti. Se un partner desidera gestire i clienti a livello globale, deve disporre di una presenza globale. Devono avere più registrazioni CSP in ognuna delle aree di vendita CSP in cui si desidera condurre le attività aziendali. Non è inoltre possibile creare account utente con accesso a tutti i tenant CSP. questo scenario potrebbe tradursi in 18 account utente per un agente di amministrazione CSP che vuole essere in grado di gestire tutti i clienti di tutto il mondo. In breve, la posizione dell'utente e dei dispositivi non è rilevante. La posizione del tenant del cliente è importante. La registrazione dei dispositivi tra i bordi non è il problema. il problema riguarda le vendite tra i confini tramite CSP. |
