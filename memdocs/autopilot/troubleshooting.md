...
Indipendentemente dal fatto che si eseguano distribuzioni di dispositivi guidate dall'utente o con distribuzione automatica, il processo di risoluzione dei problemi è analogo.  È sempre utile comprendere il flusso per un dispositivo specifico:

- Viene stabilita una connessione di rete.  La connessione può essere una connessione wireless (Wi-Fi) o cablata (Ethernet).
- Il profilo di Windows Autopilot è stato scaricato. Quando si usa una connessione cablata o si stabilisce manualmente una connessione wireless, il profilo di Windows Autopilot viene scaricato dal servizio di distribuzione Autopilot non appena viene stabilita la connessione di rete.
- Si verifica l'autenticazione utente.  Quando si esegue una distribuzione guidata dall'utente, l'utente immette le credenziali Azure Active Directory, che verranno convalidate.
- Si verifica Azure Active Directory join.  Per le distribuzioni basate sull'utente, il dispositivo verrà aggiunto al Azure AD usando le credenziali utente specificate.  Per gli scenari di distribuzione automatica, il dispositivo verrà aggiunto senza specificare le credenziali utente.
- Si verifica la registrazione automatica di MDM.  Nell'ambito del processo di Azure AD join, il dispositivo si registrerà nel servizio MDM configurato in Azure AD (ad esempio, Microsoft Intune).
- Vengono applicate le impostazioni.  Se la [pagina relativa allo stato della registrazione](enrollment-status.md) è configurata, la maggior parte delle impostazioni verrà applicata mentre viene visualizzata la pagina relativa allo stato della registrazione.  Se non è configurato o disponibile, le impostazioni verranno applicate dopo l'accesso dell'utente.

Per la risoluzione dei problemi, le attività principali da eseguire sono:

- Configurazione: ha Azure Active Directory e Microsoft Intune (o un servizio MDM equivalente) è stato configurato come specificato nei [requisiti di configurazione di Windows Autopilot](windows-autopilot-requirements.md)?
- Connettività di rete: il dispositivo può accedere ai servizi descritti nei [requisiti di rete di Windows Autopilot](windows-autopilot-requirements.md)?
...
Questo errore indica che l'hash del dispositivo non è formattato correttamente. Questo errore può essere causato da qualsiasi elemento che danneggia l'hash raccolto, ma una possibilità è che l'hash stesso (anche se è completamente valido) non riesca a essere decodificato.

L'hash del dispositivo è Base64. A livello di dispositivo, viene codificato come base64 non riempita, ma Autopilot prevede l'aggiunta di Base64. Nella maggior parte dei casi, il payload non richiede la spaziatura interna e il processo funziona. Tuttavia, a volte il payload non è allineato in modo corretto e la spaziatura interna è necessaria. Quando viene visualizzato l'errore riportato sopra. Il decodificatore Base64 di PowerShell prevede anche la codifica Base64, quindi è possibile usare questo decodificatore per verificare che l'hash venga riempito correttamente.
...
Per risolvere questo problema, è necessario modificare l'hash, quindi testare il nuovo valore fino a quando PowerShell non riesce a decodificare l'hash. Il risultato è prevalentemente illeggibile. si tratta di un'operazione che non è sufficiente per generare l'errore "lunghezza non valida per una stringa o una matrice di caratteri base-64". 

Per testare la codifica Base64, è possibile usare quanto segue:
```powershell
[System.Text.Encoding]::ascii.getstring( [System.Convert]::FromBase64String("DEVICE HASH"))
```

Quindi, come esempio (non si tratta di un hash del dispositivo, ma è non allineato con Base64, quindi è adatto per il test):
```powershell
[System.Text.Encoding]::ascii.getstring( [System.Convert]::FromBase64String("Q29udG9zbwAAA"))
```

Ora per le regole di riempimento. Il carattere di riempimento è "=". Il carattere di riempimento può essere solo alla fine dell'hash e può essere costituito solo da un massimo di 2 caratteri di riempimento. Ecco la logica di base.
...
Se il comportamento previsto di Autopilot non si verifica durante la configurazione guidata, è utile verificare se il dispositivo ha ricevuto un profilo di Autopilot e quali impostazioni sono contenute nel profilo.  A seconda della versione di Windows 10, sono disponibili diversi meccanismi a tale scopo.

### <a name="windows-10-version-1803-and-above"></a>Windows 10 versione 1803 e successive

Per visualizzare i dettagli relativi alle impostazioni del profilo di Autopilot e al flusso OOBE, Windows 10 versione 1803 e successive aggiunge voci del registro eventi.  Questi possono essere visualizzati usando Visualizzatore eventi. Esaminare le informazioni in **registri applicazioni e servizi-> Microsoft-> Windows-> provisioning-Diagnostics-provider-> Autopilot** per le versioni precedenti alla 1903. Per la versione 1903 e successive, vedere **registri applicazioni e servizi-> Microsoft-> Windows-> ModernDeployment-Diagnostics-provider-> Autopilot**.  È possibile registrare gli eventi seguenti, a seconda dello scenario e della configurazione del profilo:

| ID evento | Type | Description |
|----------|------|-------------| 
...
| 153 | Info | "AutopilotManager ha segnalato che lo stato è stato modificato da [stato originale] a [nuovo stato]".  In genere, questo messaggio deve indicare "ProfileState_Unknown" a "ProfileState_Available" per indicare che un profilo è stato disponibile per il dispositivo e scaricato, in modo che il dispositivo sia pronto per essere distribuito tramite Autopilot. |
...
| 171 | Error | "AutopilotManager non è riuscito a impostare l'identità TPM confermata.  HRESULT = [codice errore]. "  Ciò indica un problema che esegue l'attestazione TPM, necessario per completare il processo di modalità di distribuzione automatica. | 
| 172 | Error | "AutopilotManager non è riuscito a impostare il profilo di Autopilot come disponibile.  HRESULT = [codice errore]. "  Questo errore è in genere correlato all'ID evento 171. |

Oltre alle voci del registro eventi, le opzioni di traccia del registro di sistema e ETW descritte di seguito funzionano anche con Windows 10 versione 1803 e successive.

### <a name="windows-10-version-1709-and-above"></a>Windows 10 versione 1709 e successive

In Windows 10 versione 1709 e successive, le informazioni sulle impostazioni del profilo di Autopilot vengono archiviate nel registro di sistema del dispositivo dopo la ricezione dal servizio di distribuzione Autopilot.  Si trovano in **HKLM\SOFTWARE\Microsoft\Provisioning\Diagnostics\Autopilot**.  Le voci del registro di sistema disponibili includono:

| valore | Description |
|-------|-------------|
| AadTenantId | GUID del tenant Azure AD cui l'utente ha effettuato l'accesso.  Deve corrispondere al tenant con cui il dispositivo è stato registrato; Se non corrisponde all'utente, verrà visualizzato un errore. |
...
| CloudAssignedTenantId | Il GUID del tenant Azure AD il dispositivo è stato registrato (il GUID corrisponde al dominio del tenant dal valore del registro di sistema CloudAssignedTenantDomain).  Se il dispositivo non è registrato con Autopilot, questo valore sarà vuoto.|
| IsAutopilotDisabled | Se impostato su 1, indica che il dispositivo non è registrato con Autopilot.  Questo potrebbe anche indicare che non è stato possibile scaricare il profilo di Autopilot a causa di problemi di connettività di rete o di firewall o di timeout della rete. |
| TenantMatched | Questo verrà impostato su 1 se l'ID tenant dell'utente corrisponde all'ID tenant con cui è stato registrato il dispositivo.  Se questo valore è 0, all'utente viene visualizzato un errore e viene forzata la riparte. |
| CloudAssignedOobeConfig | Si tratta di una bitmap che mostra quali impostazioni di Autopilot sono state configurate.  I valori includono: SkipCortanaOptIn = 1, OobeUserNotLocalAdmin = 2, SkipExpressSettings = 4, SkipOemRegistration = 8, SkipEula = 16 |

### <a name="windows-10-semi-annual-channel-supported-versions"></a>Versioni supportate del canale semestrale di Windows 10

Nei dispositivi che eseguono una [versione supportata](https://docs.microsoft.com/windows/release-information/) del canale semestrale di Windows 10, è possibile usare la traccia ETW per acquisire informazioni dettagliate da Autopilot e dai componenti correlati.  I file di traccia ETW risultanti possono quindi essere visualizzati utilizzando Windows Performance Analyzer o strumenti simili.  Per ulteriori informazioni, vedere [il Blog sulla risoluzione dei problemi avanzata](https://blogs.technet.microsoft.com/mniehaus/2017/12/13/troubleshooting-windows-autopilot-level-300400/) .

## <a name="troubleshooting-azure-ad-join-issues"></a>Risoluzione dei problemi di Azure AD join

Il problema più comune per l'aggiunta di un dispositivo a Azure AD è relativo alle autorizzazioni di Azure AD.  Assicurarsi che [sia attiva la configurazione corretta](windows-autopilot-requirements.md) per consentire agli utenti di aggiungere dispositivi ai Azure ad.  È possibile che si verifichino errori anche se l'utente ha superato il numero di dispositivi a cui è consentito partecipare, come configurato in Azure AD.

Al momento dell'importazione viene creato un dispositivo Azure AD. è importante che l'oggetto non venga eliminato. Funge da ancoraggio di Autopilot in AAD per l'appartenenza a un gruppo e la destinazione (incluso il profilo) e può causare errori di join se viene eliminato. Una volta che l'oggetto è stato eliminato, per risolvere il problema, sarà necessario eliminare e reimportare questo hash di Autopilot, in modo da poter ricreare l'oggetto associato.
...
Vedere [questo articolo della Knowledge base](https://support.microsoft.com/help/4089533/troubleshooting-windows-device-enrollment-problems-in-microsoft-intune) per assistenza per i problemi di registrazione di Intune.  Problemi comuni includono licenze non corrette o mancanti assegnate all'utente o troppi dispositivi registrati per l'utente.
...
Se la reimpostazione di Autopilot ha esito negativo immediatamente e si verifica un errore **. Accedere con un account amministratore per vedere perché e reimpostare manualmente**. per ulteriori informazioni, vedere [risolvere i problemi di reimpostazione di Autopilot](https://docs.microsoft.com/education/windows/autopilot-reset#troubleshoot-autopilot-reset) .
...
| --- | --- |
| 1709 | Il profilo viene scaricato dopo la pagina di connessione alla rete OOBE. Questa pagina non viene visualizzata quando si usa una connessione cablata. In questo caso, il profilo viene scaricato immediatamente prima della schermata EULA. |
