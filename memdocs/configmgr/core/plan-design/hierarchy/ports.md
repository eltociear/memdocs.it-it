...
Per altre informazioni, vedere i [requisiti di accesso Internet](../../servers/deploy/configure/about-the-service-connection-point.md#bkmk_urls) per il punto di connessione del servizio.
...
A partire dalla versione 1806 è possibile spostare la libreria del contenuto in un'altra posizione di archiviazione per liberare spazio su disco rigido nei server del sito primario o di amministrazione centrale. Per altre informazioni, vedere [Configurare una raccolta contenuto remota per il server del sito](the-content-library.md#bkmk_remote).  
