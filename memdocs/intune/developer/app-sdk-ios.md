...
Impostazione  | Type  | Definizione | Necessaria?
--       |  --   |   --       |  --
ADALClientId  | Stringa  | Identificatore del client Azure AD dell'app. | Obbligatorio per tutte le app che usano MSAL. |
...
ADALLogOverrideDisabled | Boolean  | Specifica se l'SDK indirizzerà tutti i log MSAL, incluse le eventuali chiamate MSAL dall'app, al proprio file di log. L'impostazione predefinita è NO. Impostare il valore su YES se l'app imposta la richiamata al log MSAL. | Facoltativo. |
ADALCacheKeychainGroupOverride | Stringa  | Specifica il gruppo Keychain da usare per la cache MSAL al posto di "com.microsoft.adalcache". Si noti che non contiene il prefisso app-id. che verrà aggiunto alla stringa specificata in fase di runtime. | Facoltativo. |
...
La costante `IntuneMAMSaveLocationLocalDrive` deve essere usata quando l'app salva i dati in qualsiasi percorso nel dispositivo locale. Analogamente, la costante `IntuneMAMSaveLocationCameraRoll` deve essere usata se l'app salva una foto nel rullino.
...
Per altre informazioni sulle funzionalità dell'API Graph, vedere [Informazioni di riferimento sull'API Graph](/graph/overview).
...
## <a name="displaying-web-content-within-an-application"></a>Visualizzazione di contenuto Web in un'applicazione

Se l'applicazione riesce a visualizzare i siti Web all'interno di una webview, potrebbe essere necessario aggiungere la logica per evitare perdite di dati, a seconda dello scenario specifico.
...
Se l'applicazione visualizza solo i dati aziendali nella webview e gli utenti non possono accedere a siti arbitrari, non sono necessarie modifiche.
